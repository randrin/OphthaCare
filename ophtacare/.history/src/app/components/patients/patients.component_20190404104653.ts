...
export class PatientsComponent implements OnInit {
  ...
  deletePatient(patient: Patient() {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.blocked = true;
        this.userservice.deleteUser(user, this._service.getUsername()).subscribe(
          res => {
            this.blocked = false;
            this.responseData = res.json();
            if (this.responseData.code != "OK") {
              this.messageService.add({ sticky: true, severity: 'error', summary: this.responseData.code, detail: this.responseData.message });
            } else {
              this.messageService.add({ sticky: false, severity: 'success', summary: 'Confermato', detail: 'utente eliminato' });
              this.getUsers();
            }
          },
          error => {
            this.blocked = false;
            this.messageService.add({ sticky: true, severity: 'error', summary: 'Error', detail: 'Internal Error' });
          }
        );
